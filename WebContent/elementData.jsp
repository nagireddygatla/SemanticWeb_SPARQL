...
<%@ page import="com.hp.hpl.jena.query.*,com.hp.hpl.jena.tdb.*,java.util.*" %>
  ...
  String Element = request.getParameter("Element");
    ...
    if(Element.equalsIgnoreCase("ununtrium") || Element.equalsIgnoreCase("ununpentium") || Element.equalsIgnoreCase("ununseptium") || Element.equalsIgnoreCase("ununhexium") || Element.equalsIgnoreCase("ununoctium")){
    	...
    	queryString1 = prefix+ "SELECT ?name ?symbol ?atomicnumber ?stdstate ?group ?period ?block { ?element table:name ?name. ?element table:symbol ?symbol. ?element table:atomicNumber ?atomicnumber. ?element table:standardState ?stdstate. ?element table:period ?period. ?element table:group ?group. ?element table:block ?block. FILTER (str(?name) = \"" + Element + "\") } ORDER BY ?number";
    	}
      ...
      try {
		...
		%><table border = "1">
		<caption>Element - <%=Element %></caption>
		<th>Property</th>
		<th>Detail</th>
